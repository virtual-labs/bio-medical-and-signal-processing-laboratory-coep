...
var r=new Raphael(document.getElementById('axisX'),500,30);
...
var X=r.path("M0 10L1200 10").attr({
	...
	jQuery.fn.func2 = function(){
			...
			r.rect(0,15,1200,20).attr({"stroke":"#EEE", fill: "white"});
			for(i=0,cnt;i<6;i++)
			{	
				var x=i+cnt;
...
{
		var i,counter=1,y_cordinate=2;

		var r1=new Raphael(document.getElementById('axisY'),30,450);
		var X1=r1.path("M30 0L30 340").attr({
            stroke: 'green',
            'stroke-width': 4
        });
		
		for(i=20;i<350;i+=17.4)
		{
			if(counter==5)
			{
				counter=1;
				r1.path("M15 "+i+"L35 "+i).attr("stroke","green");
				r1.text(10,i-80,y_cordinate).attr({'font-size':14});
				y_cordinate--;
			}
			else
			{
		...
		r1.text(10,260,"-1").attr({'font-size':14});
		r1.text(12,340,"-1.9").attr({'font-size':14});	
}	

var drawCurve1=function(stv)
	{
			//alert(stval);
			initAll();
			var o = new Array();
			stval = stv;  //  persisting the value of subtype option
			for(var i=0;i<30;i++)
			{
				o[i] = new SetTwoD();
				o[i].setArray();
			...
			for(var i=0;i<30;i++)
			{
				//y[11] = 1.404519160130902-1.33333333;
				var z = count;
				if(o[i].y[17]<=o[i].y[6] && (stval=='lead1'||stval=='lead2'||stval=='lead3')){
					o[i].y[17] = o[i].y[17]+.2;
					o[i].y[6] = o[i].y[6]-.08;
				}
			//	o[i].y[11]=o[i].y[11]-1.33333;
	...
	var getPoints=function()
	{
	    ...
	    switch(stval){
			case 'lead1':
				y[6]=y[6]+getRandomArbitary (0.25-0.15, 0.25+0.15) ;// for P
				y[9]=y[9]-0.15;										// for Q
				y[11]=y[11]+getRandomArbitary (0.6, 0.8);				// for R
				//console.log(">>"+y[11]);
				
				y[13]=y[13]-0.25;									// for S
				y[17]=y[17]+getRandomArbitary (0.25-0.15, 0.15) ;		// for T
			break;
			
			case 'lead2':
			    y[6]=y[6]+getRandomArbitary (0.25-0.15, 0.25+0.15) ;
				y[9]=y[9]-0.15;
				y[11]=y[11]+getRandomArbitary (1.4, 1.6);
				//console.log(">>"+y[11]);
					
				y[13]=y[13]-0.25;
				y[17]=y[17]+getRandomArbitary (0.25-0.15, 0.25+0.15) ;
			break;
			
			case 'lead3':
				y[6]=y[6]+getRandomArbitary (0.25-0.15, 0.25+0.15) ;
				y[9]=y[9]-0.15;
				y[11]=y[11]+getRandomArbitary (0.8, 1.0);
				//console.log(">>"+y[11]);
				
				y[13]=y[13]-0.25;
				y[17]=y[17]+getRandomArbitary (0.25-0.15, 0.20) ;
			   
			break;
			
			case 'avr':
				y[6]=y[6]+getRandomArbitary (-0.04,-0.05) ;
				y[9]=y[9]+0.04;
				y[11]=y[11]+getRandomArbitary (-0.4, -0.3);
				//console.log(">>"+y[11]);
				
				y[13]=y[13]+0.07;
				y[17]= y[17]+getRandomArbitary (-0.14, -0.16);
			   
			break;

			case 'avl':
				y[6]=y[6]+getRandomArbitary (0.03,0.04) ;
				y[9]=y[9]+0.0;
				y[11]=y[11]+getRandomArbitary (0.22,0.25);
				//console.log(">>"+y[11]);
				
				y[13]=y[13]-0.23;
				y[17]=y[17]+getRandomArbitary (0,0) ;
			   //console.log(">>"+y[17]);
				
			break;

			case 'avf':
				y[6]=y[6]+getRandomArbitary (0.1,0.12) ; // for P
				y[9]=y[9]+0;										// for Q
				y[11]=y[11]+getRandomArbitary (0.7, 0.73);				// for R
				//console.log(">>"+y[11]);
				
				y[13]=y[13]-0.15;									// for S
				y[17]=y[17]+getRandomArbitary (0.3, 0.35) ;		// for T
			break;
			
			case 'v1':
				y[6]=y[6]+getRandomArbitary (0.07,0.09) ; 
				y[9]=y[9]+0;								
				y[11]=y[11]+getRandomArbitary (0.1, 0.12);
				//console.log(">>"+y[11]);
				
				y[13]=y[13]-0.4;						
				y[17]=y[17]+getRandomArbitary (-0.1, -0.12) ;	
			...
			case 'v2':
				y[6]=y[6]+getRandomArbitary (0.07,0.09) ; 
				y[9]=y[9]+0;								
				y[11]=y[11]+getRandomArbitary (0.3, 0.33);
				//console.log(">>"+y[11]);
				
				y[13]=y[13]-0.5;						
				y[17]=y[17]+getRandomArbitary (0.2, 0.22) ;	
			...
			case 'v3':
				y[6]=y[6]+getRandomArbitary (0.07,0.09) ; 
				y[9]=y[9]+0;								
				y[11]=y[11]+getRandomArbitary (0.52, 0.55);
				//console.log(">>"+y[11]);
			...
			case 'v4':
				y[6]=y[6]+getRandomArbitary (0.07,0.09) ; 
				y[9]=y[9]+0;								
				y[11]=y[11]+getRandomArbitary (0.58, 0.6);
				//console.log(">>"+y[11]);
...
var drawAllCurves=function()
{
	...
	timer = setInterval("jQuery.fn.func();", 200);
